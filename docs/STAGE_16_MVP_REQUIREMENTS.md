...
| Enter a filtered table manually | Five table-style rows of native Streamlit controls with a standard primary CHROM dropdown and per-row assembly-aware POS placeholder and numeric bounds in `frontend/ui.py`; authoritative chromosome and coordinate validation in `backend/vcf_processing.py::parse_manual_variants` | Verified offline |
