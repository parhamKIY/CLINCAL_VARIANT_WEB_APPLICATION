...
- Browser refresh reconnects to an active background job through an opaque,
  bounded recovery token. Completed persisted analyses reload from SQLite by
  their random analysis ID; clinical inputs and evidence are never placed in
  the URL.
...
- Stage 43 registers the complete offline suite as the selectable Testing V2
